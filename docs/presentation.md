...
| Ventana de ajustes | `tk.Toplevel` transiente centrada sobre la principal; muestra la configuración del programa. Secciones: **Tema** (Claro/Oscuro, aplica y guarda al instante), **Estilo** (Material/Neumorfismo/Skeuomorfismo, aplica y guarda al instante), **Idioma** (combobox Español/Inglés, reconstruye la UI al cambiar) y **Acerca de** (nombre, versión `APP_VERSION`, descripción, crédito del modelo de voz Supertonic 3 de Supertone Inc. con licencia OpenRAIL-M, licencia y enlaces `Enlace.TButton` que abren `REPOSITORIO_URL`, `MODELO_URL` y `MODELO_GITHUB_URL` en el navegador vía `webbrowser`). Estructurada para sumar más secciones (voz, formatos, etc.) |
